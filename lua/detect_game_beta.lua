...
local function identify_game()
    ...
    -- Nightly sometimes returns nil or 0x00 during early boot
    if not game_code or game_code == "" then
        return error("Game detection failed: game_code is nil. Emulator not fully initialized?")
    end

    if not language_code then
        return error("Game detection failed: language_code is nil. Unsupported or unreadable ROM header.")
    end

    local rom = RECOGNISED_GAMES[game_code]
    if not rom then
        return error("Unsupported Game: " .. tostring(game_code))
    end

    local offset = rom.offset[language_code]
    if not offset then
        return error("Unsupported Language: " .. tostring(language_code) .. " for game " .. tostring(game_code))
    end

    print("Detected Game: " .. game_code .. " (" .. language_code .. ")")

    -- Copy table so we don't mutate RECOGNISED_GAMES
    local result = {
        gen = rom.gen,
        offset = offset,
        version = game_code
