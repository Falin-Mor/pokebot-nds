...
-- Author: wyanido, storyzealot, Pearl8520, FalinMor_
...
function update_pointers()
    ...
    pointers = {
        ...
        items_pouch     = 0x0221D9E4 + _ROM.offset + offset_adjustment, -- 1240 bytes long
        key_items_pouch = 0x0221DEBC + _ROM.offset + offset_adjustment, -- 332 bytes long
        tms_hms_case    = 0x0221E008 + _ROM.offset + offset_adjustment, -- 436 bytes long
        medicine_pouch  = 0x0221E1BC + _ROM.offset + offset_adjustment, -- 192 bytes long
        berries_pouch   = 0x0221E27C + _ROM.offset + offset_adjustment, -- 234 bytes long

        running_shoes = 0x0221DEC5 + _ROM.offset + offset_adjustment, -- 0 before receiving

        -- Party
        party_count = 0x0221E3E8 + _ROM.offset + offset_adjustment, -- 4 bytes before first index
        party_data  = 0x0221E3EC + _ROM.offset + offset_adjustment, -- PID of first party member

        step_counter = 0x0221EB5D + _ROM.offset + offset_adjustment,
        step_cycle   = 0x0221EB5E + _ROM.offset + offset_adjustment,

        -- Location
        map_header        = 0x0223B444 + _ROM.offset + offset_adjustment,
        trainer_x         = 0x0223B448 + _ROM.offset + offset_adjustment,
        trainer_y         = 0x0223B44C + _ROM.offset + offset_adjustment,
        trainer_z         = 0x0223B450 + _ROM.offset + offset_adjustment,
        trainer_direction = 0x0223B45D + _ROM.offset + offset_adjustment, -- 0, 0x40, 0x80, 0xC0 -> Up, Left, Down, Right
        on_bike           = 0x0223B484 + _ROM.offset + offset_adjustment,
        encounter_table   = 0x0223B7B8 + _ROM.offset + offset_adjustment,
        map_matrix        = 0x0223C3D4 + _ROM.offset + offset_adjustment,

        phenomenon_x = 0x022427E8 + _ROM.offset + offset_adjustment,
        phenomenon_z = 0x022427EC + _ROM.offset + offset_adjustment,

        egg_hatching = 0x0225BB50 + _ROM.offset + offset_adjustment,

        -- Battle
        battle_indicator = 0x02258D86 + _ROM.offset + offset_adjustment, -- 0x41 if during a battle
        foe_count        = 0x02258D90 + _ROM.offset + offset_adjustment, -- 4 bytes before the first index
        current_foe      = 0x02258D94 + _ROM.offset + offset_adjustment, -- PID of foe, set immediately after the battle transition ends
		foe_species		 = species_base,
		maxHP    		 = species_base + 0x2,
		currentHP		 = species_base + 0x4,
		foe_status  	 = status_base,
		
        -- Misc
        save_indicator            = 0x0223B4F0 + _ROM.offset + offset_adjustment, -- 1 while save menu is open
