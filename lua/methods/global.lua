...
function false_swipe_until_threshold(hp_before, max_hp, fs_slot)

    while true do
        emu.frameadvance()
		local mon = foe[1]
        local hp_now = mword(pointers.currentHP)
		local max = mword(pointers.maxHP)
		
        if hp_now <= max * 0.2 then
            print("[FS] HP below 20%, subdue complete")
            return true
        end

        if hp_now <= 1 then
            print("[FS] Foe at 1 HP, done")
            return true
        end

        print("[FS] Retrying False Swipe...")
        use_move(fs_slot)
        advance_battle_ui()
    end
end

function subdue_false_swipe()

    if not config.subdue_target then
        return false
    end
	
    local recoil_moves = {
        "Brave Bird","Double-Edge","Flare Blitz","Head Charge","Head Smash",
        "Self-Destruct","Take Down","Volt Tackle","Wild Charge","Wood Hammer"
    }

    for _, move in ipairs(recoil_moves) do
        if pokemon.get_move_slot(foe[1], move) ~= 0 then
            print_warn("Target has recoil. Skipping False Swipe.")
            return false
        end
    end

    local lead = get_lead_mon_index()
    local fs_slot = pokemon.get_move_slot(party[lead], "False Swipe")

    if fs_slot == 0 then
        print_warn("Lead has no False Swipe.")
        return false
    end
    ...
    use_move(fs_slot)
    advance_battle_ui()
    return false_swipe_until_threshold(nil, nil, fs_slot)
end

function status_until_hit(slot)
    while true do
        emu.frameadvance()

        local status_now = mbyte(pointers.foe_status)

        if status_now ~= 0 then
            print("[STATUS] Status applied, done")
            return true
        end

        print("[STATUS] Retrying status move: " .. slot)
        use_move(slot)
        advance_battle_ui()
    end
end

function subdue_status()
    ...
    local status_moves = {
        "Spore","Sleep Powder","Lovely Kiss","Dark Void","Hypnosis","Sing",
        "Grass Whistle","Thunder Wave","Glare","Stun Spore"
    }

    -- Remove Thunder Wave vs Ground
    for _, t in ipairs(foe[1].type) do
        if t == "Ground" then
            table.remove(status_moves, 8)
            break
        end
    end

    -- Remove Grass moves vs Sap Sipper
    if foe[1].ability == "Sap Sipper" then
        local grass = {"Spore","Sleep Powder","Grass Whistle","Stun Spore"}
    ...
    for _, move in ipairs(status_moves) do
        slot = pokemon.get_move_slot(party[lead], move)
        if slot ~= 0 then break end
    end

    if slot == 0 then
        print_warn("Lead has no usable status moves.")
        return false
    end

    print("Using status move: " .. slot)
    use_move(slot)
    advance_battle_ui()
    return status_until_hit(slot)
end
...
function save_game()
    print("Saving game...")   
    open_menu("Save")
    press_sequence("A", 90, "A", 90, "A", 90, "A", 90, "A", 800)
